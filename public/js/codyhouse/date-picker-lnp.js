...
(function () {
    ...
    function disablePastDates(datePicker) {
        var dateInput = document.getElementById('date-input-1').value;
        var disableDate = new Date(dateInput);
        var buttons = datePicker.body.getElementsByTagName("button");
        for (var i = 0; i < buttons.length; i++) {
            var buttonDate = new Date(datePicker.currentYear, datePicker.currentMonth, buttons[i].textContent);
            if (buttonDate < disableDate) {
                buttons[i].setAttribute('disabled','disabled');
                buttons[i].setAttribute('tabindex', '-1');
                var parentLi = buttons[i].closest('li');
                if (parentLi) {
                    parentLi.classList.add('date-picker__date--disabled');
                }
            }
        }
    }
            ...
            for (var i = 0; i < 6; i++) {
                for (var j = 0; j < 7; j++) {
                    if (i === 0 && j < firstDay) {
                        ...
                        // Add the 'disabled' class to past dates
                        if (date < datePicker.currentDay && getCurrentMonth() == datePicker.currentMonth && getCurrentYear() == datePicker.currentYear) {
    ...
    function getDayOfWeek(year, month, day) {
        var weekDay = (new Date(year, month, day)).getDay(); // Sunday is represented as 0
        return weekDay;
    }
...
const dpLoaded = new Event('dpLoaded');
document.dispatchEvent(dpLoaded);
